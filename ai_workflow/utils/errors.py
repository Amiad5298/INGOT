"""Custom exceptions and exit codes for AI Workflow.
...
class AIWorkflowError(Exception):
    """Base exception for AI Workflow errors.
...
class AuggieNotInstalledError(AIWorkflowError):
...
class JiraNotConfiguredError(AIWorkflowError):
...
class UserCancelledError(AIWorkflowError):
...
class GitOperationError(AIWorkflowError):
